defmodule AiposWeb.OrganizationLive.Edit do
  ...
  def handle_event("save", %{"organization" => organization_params}, socket) do
    Logger.info("Updating organization with params: #{inspect(organization_params)}")
    ...
    organization_params =
      if uploaded_files != [] do
        Map.put(organization_params, "logo", List.first(uploaded_files))
      else
        organization_params
      end

    Logger.info("Updating organization with final params: #{inspect(organization_params)}")
    ...
    case Organizations.update_organization(socket.assigns.organization, organization_params) do
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        Logger.error("Failed to update organization: #{inspect(changeset)}")

        {:noreply,
         socket
         |> put_flash(:error, "Error updating organization. Please check the form.")
         |> assign(:form, to_form(changeset))}
    end
  end
                  ...
                  <div class="col-span-2 space-y-4">
                    ...
                    <%= if @organization.logo do %>
                      <div class="mb-4">
                        ...
                        <img src={@organization.logo} alt="Current logo" class="h-24 w-24 object-cover rounded" />
                      </div>
                    <% end %>
